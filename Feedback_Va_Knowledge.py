...
st.markdown("""
<style>
/* Target the specific button with text 'X' directly.
   This leverages that it's likely the first and possibly only button of this style
   in its immediate context on the landing page.
   The data-testid="stButton" is generated by Streamlit for its buttons.
*/
button[data-testid="stButton"] > div > span {
    /* Apply styles only if the text content is 'X' AND it's the specific button */
    /* CSS does not have a direct way to target based on text content 'X' inside span without JS.
       So, we rely on the button with the label 'X' being the first one, or unique enough.
       Let's try a very high specificity for the first st.button.
    */
}

/* This targets the *first* Streamlit button on the page. */
/* It's effective for the landing page 'X' button because it's the first button rendered. */
